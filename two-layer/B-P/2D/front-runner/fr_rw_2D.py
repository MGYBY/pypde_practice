...
def disturbed_depth(x, y, normal_depth, rad, dist_amp):
    if (((x-3.0*rad)/rad)**2.0+(y/rad)**2.0<=1.0):
        return (normal_depth*(1.0+dist_amp*(1.0-((x-3.0*rad)/rad)**2.0-(y/rad)**2.0)**0.50))
    else:
        return (normal_depth)
...
for i in range(nx):
    for j in range(ny):
        ...
        Q0[i, j, 0] = disturbed_depth(x_coord, y_coord, nd1, dist_rad_val, dist_amp_val)
        Q0[i, j, 1] = disturbed_depth(x_coord, y_coord, nd2, dist_rad_val, dist_amp_val)
        # constant-Fr setups
        Q0[i, j, 2] = nv1*(Q0[i, j, 0]/(nd1))**0.50*Q0[i, j, 0]
        Q0[i, j, 3] = nv2*(Q0[i, j, 1]/(nd2))**0.50*Q0[i, j, 1]
        # Q0[i, j, 2] = nv1*(nd1)
        # Q0[i, j, 3] = nv2*(nd2)
        Q0[i, j, 4] = 0.0
        Q0[i, j, 5] = 0.0
...
out = pde_solver(Q0, tf, L, F=F, S=S, B=B_dl, boundaryTypes='periodic', cfl=0.808, order=1, stiff=False, flux='rusanov', ndt=n_output, nThreads=10)
