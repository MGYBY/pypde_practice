...
def selected_indices(times, requested_times):
...
def time_tag(value):
    return f"t_{value:012.5f}".replace("-", "m").replace(".", "p")
...
def postprocess_solution(
    *,
    ...
    """Write the complete history, diagnostics, snapshots, and figures."""
    ...
    if out.ndim != 3 or out.shape[1:] != Q0.shape:
        raise ValueError("out must have shape (nt, nx, 5).")
    if times.shape != (out.shape[0],) or x.shape != (Q0.shape[0],):
        raise ValueError("Inconsistent time or cell-centre arrays.")
    ...
    chosen = selected_indices(all_times, snapshot_times)
    chosen_set = set(chosen.tolist())

    np.savez_compressed(root / "complete_solution.npz", x=x, times=all_times, states=all_states)
    ...
    for frame, time_value in enumerate(all_times):
        ...
        if frame in chosen_set:
            selected_data[frame] = (raw, closure)
    ...
    np.savetxt(root / "diagnostics_time_series.txt", diagnostics, fmt="%.12e", delimiter="\t", header=diagnostic_header)
    ...
    for frame in chosen:
        time_value = float(all_times[frame])
        raw, closure = selected_data[int(frame)]

        if save_snapshot_text:
            table = snapshot_table(x, raw, closure, packing)
            np.savetxt(
    ...
    if save_plots:
        ...
        for axis in axes:
            axis.grid(True, alpha=0.25)
        finish_figure(fig, figure_dir / "extrema_h_b_time_series.png", plt, dpi, show_plots)
        ...
        for axis in axes:
            axis.grid(True, alpha=0.25)
        finish_figure(fig, figure_dir / "diagnostics_time_series.png", plt, dpi, show_plots)

    print("Postprocessing directory:", root)
    print("Saved snapshot times:", [float(all_times[i]) for i in chosen])
