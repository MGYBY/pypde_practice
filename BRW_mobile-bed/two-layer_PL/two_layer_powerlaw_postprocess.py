...
# Number of evenly spaced frames used for figures, including t=0 and t=tf.
NUMBER_OF_PLOTTED_FRAMES = 5
PLOT_DPI = 220
SHOW_PLOTS = False
...
def _select_plot_indices(number_of_frames: int) -> np.ndarray:
    count = min(NUMBER_OF_PLOTTED_FRAMES, number_of_frames)
    return np.unique(
...
def postprocess_solution(
    *,
    ...
    times = np.concatenate(
        (
            np.array([0.0]),
            np.arange(1, out.shape[0] + 1, dtype=float)
            * final_time
            / out.shape[0],
        )
    )
