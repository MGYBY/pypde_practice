...
# Requested spacing between saved positive-time states.  The initial state
# t=0 is handled separately and is not counted by PyPDE's ``ndt`` argument.
OUTPUT_INTERVAL = 1.0

# ``nominal_uniform``: one efficient PyPDE call.  Frames are associated with
# the uniformly spaced target times, although the stock C++ backend stores the
# first internal state after each target is crossed.
#
# ``exact_segmented``: one PyPDE call per requested interval, each with ndt=1.
# PyPDE clips the last CFL step to each segment end, so the returned states are
# exactly at the requested times.  This is more expensive because the solver
# and callback wrappers are rebuilt for every segment.
OUTPUT_TIME_MODE = "nominal_uniform"


def _requested_output_times() -> np.ndarray:
    if OUTPUT_INTERVAL <= 0.0:
        raise ValueError("OUTPUT_INTERVAL must be positive.")

    number_of_intervals = int(round(FINAL_TIME / OUTPUT_INTERVAL))
    if number_of_intervals < 1 or not np.isclose(
        number_of_intervals * OUTPUT_INTERVAL,
        FINAL_TIME,
        rtol=0.0,
        atol=1.0e-12 * max(1.0, abs(FINAL_TIME)),
    ):
        raise ValueError(
            "FINAL_TIME must be an integer multiple of OUTPUT_INTERVAL. "
            f"Received FINAL_TIME={FINAL_TIME} and "
            f"OUTPUT_INTERVAL={OUTPUT_INTERVAL}."
        )

    return OUTPUT_INTERVAL * np.arange(1, number_of_intervals + 1, dtype=float)


OUTPUT_TIMES = _requested_output_times()
N_OUTPUTS = int(OUTPUT_TIMES.size)
...
def _validate_parameters() -> None:
    ...
    if FINAL_TIME <= 0.0 or N_OUTPUTS < 1:
        raise ValueError("FINAL_TIME and the number of outputs must be positive.")
    if OUTPUT_TIME_MODE not in {"nominal_uniform", "exact_segmented"}:
        raise ValueError(
            "OUTPUT_TIME_MODE must be 'nominal_uniform' or "
            "'exact_segmented'."
        )
...
def _pypde_call(pde_solver, state: np.ndarray, duration: float, ndt: int) -> np.ndarray:
    """Call PyPDE using a private contiguous state buffer.

    PyPDE evolves the array passed as ``Q0`` in place.  Passing a private copy
    prevents the user-facing initial condition from being overwritten.
    """
    solver_state = np.ascontiguousarray(state, dtype=float).copy()
    result = pde_solver(
        solver_state,
        float(duration),
        [DOMAIN_LENGTH],
...
def _run_at_requested_times(pde_solver, Q_initial: np.ndarray) -> np.ndarray:
    """Return states associated with ``OUTPUT_TIMES``."""
    if OUTPUT_TIME_MODE == "nominal_uniform":
        return _pypde_call(
            pde_solver,
            Q_initial,
            FINAL_TIME,
    ...
    # Exact-output workaround for the stock PyPDE API.  The equations in this
    # model are autonomous, so restarting at each output time is equivalent to
    # continuing the same evolution, apart from roundoff and repeated startup.
    frames = []
    state = np.array(Q_initial, dtype=float, copy=True, order="C")
    previous_time = 0.0

    for target_time in OUTPUT_TIMES:
        segment_duration = float(target_time - previous_time)
        segment = _pypde_call(pde_solver, state, segment_duration, ndt=1)
        state = np.array(segment[-1], dtype=float, copy=True, order="C")
        frames.append(state)
...
def main() -> None:
    ...
    # Preserve an immutable user-facing copy.  The stock PyPDE wrapper passes
    # its input buffer to C++, which updates that buffer in place.
    Q_initial = np.array(initial_condition(x), dtype=float, copy=True, order="C")

    print_parameter_summary()
    print(f"output interval               = {OUTPUT_INTERVAL:.10g}")
    print(f"positive-time output frames   = {N_OUTPUTS}")
    print(f"output-time mode              = {OUTPUT_TIME_MODE}")

    OUTPUT_DIRECTORY.mkdir(parents=True, exist_ok=True)
    np.save(OUTPUT_DIRECTORY / f"{OUTPUT_STEM}_x.npy", x)
    np.save(OUTPUT_DIRECTORY / f"{OUTPUT_STEM}_Q0.npy", Q_initial)
    np.save(OUTPUT_DIRECTORY / f"{OUTPUT_STEM}_times.npy", OUTPUT_TIMES)

    out = _run_at_requested_times(pde_solver, Q_initial)

    expected_shape = (N_OUTPUTS, NX, 4)
    if out.shape != expected_shape:
        raise RuntimeError(
            f"Unexpected PyPDE output shape {out.shape}; "
            f"expected {expected_shape}."
        )

    np.save(OUTPUT_DIRECTORY / f"{OUTPUT_STEM}_out.npy", out)
    np.savez_compressed(
        OUTPUT_DIRECTORY / f"{OUTPUT_STEM}_complete_solution.npz",
        x=x,
        times=np.concatenate(([0.0], OUTPUT_TIMES)),
        states=np.concatenate((Q_initial[None, :, :], out), axis=0),
        initial_state=Q_initial,
        solution=out,
    )

    if RUN_POSTPROCESSING:
        from two_layer_powerlaw_postprocess_output_fixed import postprocess_solution
