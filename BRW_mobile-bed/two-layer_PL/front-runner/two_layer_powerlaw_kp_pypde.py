#!/usr/bin/env python3
"""Two-layer power-law Kármán–Pohlhausen PyPDE model with front-runner support.
...
The user-facing rheological contrast is

    R_eta,I = eta_lower(interface) / eta_upper(interface)

evaluated in the steady-uniform base state.  The former scaled consistency
ratio kappa_K is derived internally from R_eta,I and the unchanged depth,
density and power-law-index ratios.

Compared with the earlier implementation, this edition:

* replaces kappa_K as an independent input by the lower-over-upper local
  interfacial apparent-viscosity ratio R_eta,I;
...
# Independent set: (Fr_l, h_r, r_rho, n_l, n_u, R_eta,I), with
# R_eta,I = eta_l,I/eta_u,I in the steady-uniform base state.
FR_LOWER = 0.40
DEPTH_RATIO = 1.00
DENSITY_RATIO = 0.80
N_LOWER = 0.40
N_UPPER = 0.80
# R_eta,I = eta_l,I/eta_u,I on the steady-uniform interface.  This default
# is exactly equivalent to the former kappa_K=1 default.
INTERFACIAL_APPARENT_VISCOSITY_RATIO = 1.5439718929919697
...
# --- Front-runner case loader ------------------------------------------------
# Case parameters are read once, BEFORE material constants are derived. Do not
# change physical globals in an imported/compiled module; start a new process.
from front_runner_config import load_case_settings
_CASE, _CASE_PATH, _CASE_OVERRIDES = load_case_settings()
globals().update(_CASE_OVERRIDES)
...
class DerivedModel:
    ...
    uniform_a: float
    uniform_b: float
    interfacial_shear_factor: float
    scaled_consistency_ratio: float
    uniform_eta_lower_interface: float
    uniform_eta_upper_interface: float
    lambda_lower: float
    lambda_upper: float
    uniform_tau_b: float
    uniform_tau_i: float
...
def _validate_parameters() -> None:
    finite_positive = {
        ...
        "INTERFACIAL_APPARENT_VISCOSITY_RATIO": INTERFACIAL_APPARENT_VISCOSITY_RATIO,
    ...
    if not FRONT_RUNNER_ENABLED and not np.isclose(n_intervals, round(n_intervals), rtol=0.0, atol=2.0e-12):
        raise ValueError("FINAL_TIME must be an integer multiple of OUTPUT_INTERVAL.")

    if BOUNDARY_TYPE == "periodic" and INITIAL_CONDITION_MODE != "front_runner_constant_froude":
...
def interfacial_shear_factor(
    depth_ratio: float = DEPTH_RATIO,
    density_ratio: float = DENSITY_RATIO,
    lower_index: float = N_LOWER,
) -> float:
    """Return Phi_I = gamma_l,I H_l/U_l for the normal-flow state."""
    if depth_ratio <= 0.0 or density_ratio <= 0.0 or lower_index <= 0.0:
        raise ValueError("depth_ratio, density_ratio and lower_index must be positive.")
    lam = density_ratio * depth_ratio / (1.0 + density_ratio * depth_ratio)
    m_l = (lower_index + 1.0) / lower_index
    c_l, a_l, _ = _lower_shape_coefficients_core(lam, m_l)
    return lam ** (1.0 / lower_index) / (a_l * c_l)


def kappa_from_interfacial_viscosity_ratio(
    R_eta_I: float,
    depth_ratio: float = DEPTH_RATIO,
    density_ratio: float = DENSITY_RATIO,
    lower_index: float = N_LOWER,
    upper_index: float = N_UPPER,
) -> float:
    """Return kappa_K for R_eta,I=eta_l,I/eta_u,I."""
    if not np.isfinite(R_eta_I) or R_eta_I <= 0.0:
        raise ValueError("R_eta_I must be finite and positive.")
    phi = interfacial_shear_factor(depth_ratio, density_ratio, lower_index)
    return R_eta_I ** (-upper_index) * phi ** (lower_index - upper_index)


def interfacial_viscosity_ratio_from_kappa(
    kappa_K: float,
    depth_ratio: float = DEPTH_RATIO,
    density_ratio: float = DENSITY_RATIO,
    lower_index: float = N_LOWER,
    ...
    """Return R_eta,I=eta_l,I/eta_u,I for a former kappa_K input."""
    if not np.isfinite(kappa_K) or kappa_K <= 0.0:
        raise ValueError("kappa_K must be finite and positive.")
    phi = interfacial_shear_factor(depth_ratio, density_ratio, lower_index)
    return (phi ** (lower_index - upper_index) / kappa_K) ** (1.0 / upper_index)
...
def _derive_model() -> DerivedModel:
    ...
    phi_i = uniform_lambda ** (1.0 / N_LOWER) / (a0 * c0)
    kappa_k = (
        INTERFACIAL_APPARENT_VISCOSITY_RATIO ** (-N_UPPER)
        * phi_i ** (N_LOWER - N_UPPER)
    )
    ...
    lambda_u = kappa_k * lambda_l

    tau_b0 = (1.0 + DENSITY_RATIO * DEPTH_RATIO) / FR_LOWER**2
    tau_i0 = DENSITY_RATIO * DEPTH_RATIO / FR_LOWER**2
    ui0 = 1.0 / a0
    gamma_l_i0 = phi_i
    gamma_u_i0 = INTERFACIAL_APPARENT_VISCOSITY_RATIO * phi_i
    eta_l_i0 = tau_i0 / gamma_l_i0
    eta_u_i0 = tau_i0 / gamma_u_i0
    w0 = DEPTH_RATIO * gamma_u_i0 / m_u
    uu0 = ui0 + a_u * w0
    qu0 = DEPTH_RATIO * uu0
    ...
    return DerivedModel(
        ...
        uniform_a=a0,
        uniform_b=b0,
        interfacial_shear_factor=phi_i,
        scaled_consistency_ratio=kappa_k,
        uniform_eta_lower_interface=eta_l_i0,
        uniform_eta_upper_interface=eta_u_i0,
...
INTERFACIAL_SHEAR_FACTOR = DERIVED.interfacial_shear_factor
SCALED_CONSISTENCY_RATIO = DERIVED.scaled_consistency_ratio
UNIFORM_ETA_LOWER_INTERFACE = DERIVED.uniform_eta_lower_interface
UNIFORM_ETA_UPPER_INTERFACE = DERIVED.uniform_eta_upper_interface
...
def output_times() -> np.ndarray:
    if FRONT_RUNNER_ENABLED:
        from front_runner_initialization import scheduled_times
        return scheduled_times(FINAL_TIME, OUTPUT_INTERVAL)
...
def initial_condition(
    ...
    *,
    ...
    if selected_mode == "front_runner_constant_froude":
        from front_runner_initialization import constant_froude_state
        return constant_froude_state(
            x, q0, amplitude=selected_amplitude,
            wavelength=PERTURBATION_WAVELENGTH,
            center=FR_CENTER_WAVELENGTHS * PERTURBATION_WAVELENGTH,
        )
...
def print_parameter_summary(preflight: Optional[Dict[str, float]] = None) -> None:
    ...
    print("\nAudited two-layer power-law Kármán–Pohlhausen model (R_eta,I input)")
    print("----------------------------------------------------------------")
    ...
    print(f"R_eta,I = eta_l,I/eta_u,I         = {INTERFACIAL_APPARENT_VISCOSITY_RATIO:.10g}")
    print(f"derived scaled consistency ratio  = {SCALED_CONSISTENCY_RATIO:.10g}")
    print(f"Phi_I = gamma_l,I H_l/U_l         = {INTERFACIAL_SHEAR_FACTOR:.10g}")
    print(f"Lambda_l, Lambda_u                = {LAMBDA_LOWER:.10g}, {LAMBDA_UPPER:.10g}")
    print(f"normal-flow eta_l,I/eta_u,I       = {UNIFORM_ETA_LOWER_INTERFACE/UNIFORM_ETA_UPPER_INTERFACE:.10g}")
...
def main() -> None:
    # Use the current module rather than importing a second, differently
    # configured copy during postprocessing when invoked as __main__.
    import sys
    from front_runner_runtime import run_case
    run_case(sys.modules[__name__])
