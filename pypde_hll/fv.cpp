...
void FVSolver::interfaces(Matr u, Matr qh, double dt) {
  ...
  Vec f = Vec::Zero(V);
  Vec b = Vec::Zero(V);
  Vec hllLeft = Vec::Zero(V);
  Vec hllRight = Vec::Zero(V);
    ...
    for (int t = 0; t < N; t++) {
      ...
      for (int d = 0; d < ndim; d++) {
        ...
        for (int idx = 0; idx < Nd_; idx++) {
          ...
          // TODO: fix dq0, dq1
          if (FLUX == HLL) {
            fluxGenerator.hll_fluxes(
                hllLeft, hllRight, q0.row(idx), q1.row(idx),
                dq0.block(idx * ndim, 0, ndim, V),
                dq1.block(idx * ndim, 0, ndim, V), d, secondOrder);
          } else {
            if (F != NULL)
              fluxGenerator.flux(
                  f, q0.row(idx), q1.row(idx),
                  dq0.block(idx * ndim, 0, ndim, V),
                  dq1.block(idx * ndim, 0, ndim, V), d, secondOrder);
          ...
          if (FLUX == HLL) {
            if (indexer0.valid)
              u.row(indexer0.u) -= tmp * hllLeft;
            if (indexer1.valid)
              u.row(indexer1.u) += tmp * hllRight;
          } else {
            if (indexer0.valid)
