...
public:
  ...
  // Path-conservative HLL interface contributions.  retL is applied to the
  // cell on the left with a minus sign and retR to the cell on the right with
  // a plus sign.  Both are returned with the factor of two used by fv.cpp.
  void hll_fluxes(Vecr retL, Vecr retR, Vecr qL, Vecr qR, Matr dqL,
                  Matr dqR, int d, bool secondOrder);
};
